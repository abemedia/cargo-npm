    ...
    if let Some(custom) = &job.meta.custom {
        for (k, v) in custom {
            match k.as_str() {
                ...
                k @ ("bin" | "optionalDependencies") => {
                    let Value::Object(new) = v else {
                        bail!("custom field \"{k}\" must be an object");
                    };
                    if !pkg_json[k].is_object() {
                        pkg_json[k] = json!({});
                    }
                    let existing = pkg_json[k].as_object_mut().unwrap();
                    for (key, val) in new {
                        if existing.insert(key.clone(), val.clone()).is_some() {
                            bail!("custom field \"{k}.{key}\" would overwrite a generated value");
                        }
                    }
                }
